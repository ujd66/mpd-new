...
class DatasetNormalizer:
    ...
    def grad_x_wrt_x_normalized(self, x, key):
        return self.normalizers[key].grad_x_wrt_x_normalized(x)
...
def flatten(dataset):
    ...
    flattened = {}
    for key, xs in dataset.items():
        xs_new = einops.rearrange(xs, "... d -> (...) d")
...
class Normalizer:
    ...
    def grad_x_wrt_x_normalized(self, *args, **kwargs):
        raise NotImplementedError
...
class LimitsNormalizer(Normalizer):
    ...
    def grad_x_wrt_x_normalized(self, x, eps=1e-4):
        # faster gradient computation than autograd
        mask = (x >= -1 - eps) & (x <= 1 + eps)
        grad = torch.ones_like(x) * mask
        return grad * (self.maxs - self.mins) / 2.0
