...
class STOMP(OptimizationPlanner):
    ...
    def _get_R_mat(self):
        ...
        A_mat[0, 0] = 1.0
        A_mat[-1, -1] = 1.0
        A_mat = A_mat * 1.0 / self.dt**2 * self.sigma_spectral
        R_mat = A_mat.t() @ A_mat
    ...
    def sample(self):
        ...
        """
        noise = (
            self._noise_dist.sample((self.num_samples, self.d_state_opt))
            .transpose(0, 2)
            .transpose(1, 3)
            .transpose(1, 2)
        )  # [num_particles, num_samples, n_support_points, n_dof]
    ...
    def const_vel_trajectory(
        ...
        for i in range(num_steps + 1):
            state_traj[i, : self.n_dof] = (
                start_state[: self.n_dof] * (num_steps - i) * 1.0 / num_steps
                + goal_state[: self.n_dof] * i * 1.0 / num_steps
            )
    ...
    def _update_distribution(self, costs, traj_particles):
        ...
        self._particle_means.add_(
            self.lr * self.Sigma @ (self._weights * (traj_particles - self._particle_means.unsqueeze(1))).sum(1)
        )
