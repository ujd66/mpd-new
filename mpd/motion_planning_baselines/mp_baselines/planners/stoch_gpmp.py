...
class StochGPMP(OptimizationPlanner):
    ...
    def const_vel_trajectories(
        ...
        for i in range(self.n_support_points):
            interp_state = start_state[: self.n_dof] * (self.n_support_points - i - 1) / (
                self.n_support_points - 1
    ...
    def _get_costs(self, **observation):
        ...
        V = self.state_samples.view(
            -1, self.num_samples, self.n_support_points * self.d_state_opt
    ...
    def _update_distribution(self, costs, traj_samples):
        ...
        self._particle_means.add_(self.step_size * approx_grad)
        self._sample_dist.set_mean(self._particle_means.view(self.num_particles, -1))

        return approx_grad

    def optimize(self, opt_iters=None, debug=False, **observation):
        ...
        for opt_step in range(opt_iters):
            with torch.no_grad():
                (
                    ...
                    state_particles,
                    costs,
                ) = self.sample_and_eval(**observation)
    ...
    def sample_trajectories(self, num_samples_per_particle):
        self._sample_dist.set_mean(self._particle_means.view(self.num_particles, -1))
        self.state_samples = self._sample_dist.sample(num_samples_per_particle).to(**self.tensor_args)
        position_seq = self.state_samples[..., : self.n_dof]
        velocity_seq = self.state_samples[..., -self.n_dof :]
        return (
            position_seq,
