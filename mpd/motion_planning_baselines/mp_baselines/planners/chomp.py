...
class CHOMP(OptimizationPlanner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.Sigma_inv = self._get_R_mat(
            dt=self.dt, n_support_points=self.n_support_points, tensor_args=self.tensor_args
        )
    ...
    def _get_R_mat2(self):
        ...
        lower_diag = torch.diag(
            torch.ones(self.n_support_points - 1),
            diagonal=-1,
        )
        diag = -2 * torch.eye(self.n_support_points)
        A_mat = upper_diag + diag + lower_diag
        A_mat = torch.cat(
            (torch.zeros(1, self.n_support_points), A_mat, torch.zeros(1, self.n_support_points)),
            dim=0,
        )
        A_mat[0, 0] = 1.0
        A_mat[-1, -1] = 1.0
        A_mat = A_mat * 1.0 / self.dt**2
        R_mat = A_mat.t() @ A_mat
        return R_mat.to(**self.tensor_args)

    @classmethod
    def _get_R_mat(cls, dt=0.01, n_support_points=64, tensor_args=DEFAULT_TENSOR_ARGS, **kwargs):
        """
    ...
    def _run_optimization(self, opt_iters, **observation):
        ...
        for opt_step in range(opt_iters):
            ...
            self._particle_means.grad.data[..., 0, :] = 0.0
            self._particle_means.grad.data[..., -1, :] = 0.0
