...
def avg_ctrl_to_goal(
    ...
    if control_type == "velocity":
        return ((target - state) / rollout_steps / dt).clamp(max=max_ctrl)
    else:
        state_dim = state.dim()
        pos_dim = int(state_dim / 2)
        return ((target[:pos_dim] - state[:pos_dim]) / rollout_steps / dt**2).clamp(max=max_ctrl)
...
def get_multivar_gaussian_prior(
    ...
    return ControlTrajectoryGaussian(rollout_steps, control_dim, mu, Cov, tensor_args=tensor_args)
...
def const_ctrl_Cov(
    ...
    LL_t = torch.matmul(L, L.transpose(0, 1))
    ...
    for i in range(Cov.shape[-1]):
        ...
        if det < 1.0e-7:
            raise ZeroDivisionError(
                "Covariance-determinant too small, potential for underflow.  " "Consider increasing sigma."
            )
...
class ControlTrajectoryPrior(ABC):
    ...
    def log_prob(self, samples, cond_inputs=None):
        """
        ...
        for i in range(self.ctrl_dim):
            ...
            log_probs += self.list_ctrl_dists[i].log_prob(samp)
    ...
    def sample(
        ...
        ctrl_dim)
        """
        U_s = torch.empty(num_samples, self.rollout_steps, self.ctrl_dim, **self.tensor_args)
        # TODO - remove this for loop
        for i in range(self.ctrl_dim):
            U_s[:, :, i] = self.list_ctrl_dists[i].sample((num_samples,)).to(**self.tensor_args)
        return U_s
...
class ControlTrajectoryGaussian(ControlTrajectoryPrior):
    ...
    def make_dist(self):
        for i in range(self.ctrl_dim):
            self.list_ctrl_dists.append(dist.MultivariateNormal(self.mu[:, i], covariance_matrix=self.Cov[:, :, i]))
