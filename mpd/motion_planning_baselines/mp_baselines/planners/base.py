...
class MPPlanner(ABC):
    ...
    def __init__(self, name: str = None, tensor_args: dict = None, **kwargs):
        self.name = name
...
class OptimizationPlanner(MPPlanner):
        ...
        if self.pos_only:
            ...
            if self.multi_goal_states is not None:
                self.multi_goal_states = torch.cat(
                    [self.multi_goal_states, torch.zeros_like(self.multi_goal_states)], dim=-1
                )
    ...
    def const_vel_trajectories(
        ...
        mean_vel = (multi_goal_states[:, : self.n_dof] - start_state[:, : self.n_dof]) / (
            self.n_support_points * self.dt
        )
        for i in range(self.n_support_points):
            state_traj[:, i, : self.n_dof] = start_state[:, : self.n_dof] * (self.n_support_points - i - 1) / (
                self.n_support_points - 1
