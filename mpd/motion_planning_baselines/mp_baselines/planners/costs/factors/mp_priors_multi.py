...
class MultiMPPrior:
    ...
    def __init__(
        ...
        if means is None:
            ...
            means = self.get_const_vel_mean(start_state, goal_states, dt, num_steps, dof)
        else:
    ...
    def const_vel_trajectory(
        ...
        for i in range(num_steps + 1):
            state_traj[i, :dof] = (
                start_state[:dof] * (num_steps - i) * 1.0 / num_steps + goal_state[:dof] * i * 1.0 / num_steps
            )
    ...
    def get_const_vel_mean(
        ...
        if self.goal_directed:
            ...
            for i in range(self.num_modes):
                means.append(
                    self.const_vel_trajectory(
                        start_state, goal_states[i], dt, num_steps, dof, tensor_args=self.tensor_args
                    )
                )
    ...
    def get_const_vel_covariance(
        ...
        A[self.state_dim :, : -self.state_dim] += -1.0 * diag_Phis
        if self.goal_directed:
