...
class MPPI(MPPlanner):
    ...
    def update_controller(self, costs, U_sampled):
        ...
        self._mean.add_(self.step_size * (weights * (U_sampled - self._mean.unsqueeze(0))).sum(0))
    ...
    def sample_and_eval(self, **observation):
        ...
        self.costs = self.system.traj_cost(
            state_trajectories.transpose(0, 1).unsqueeze(2), control_samples.transpose(0, 1).unsqueeze(2), **observation
        )
        ...
        return (control_samples, state_trajectories, self.costs)
