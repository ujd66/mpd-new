...
def build_gpmp2_cost_composite(
    ...
    **kwargs,
    ...
    cost_gp_prior = CostGP(robot, n_support_points, start_state_zero_vel, dt, cost_sigmas, tensor_args=tensor_args)
...
class GPMP2(OptimizationPlanner):
    ...
    def _step(self, debug=False, **observation):
        with TimerCUDA() as t_grad:
            A, b, K = self.cost.get_linear_system(
                self._particle_means, n_interpolated_points=self.n_interpolated_points, **observation
            )
        # if debug:
    ...
    def _get_grad_terms(
        self, A, b, K, delta=0.0, trust_region=False, sparse_computation=False, sparse_computation_block_diag=False
    ):
