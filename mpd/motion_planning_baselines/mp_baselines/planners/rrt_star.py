...
class RRTStar(RRTBase):
        ...
        with TimerCUDA() as t:
            while (t.elapsed < self.max_time) and (iteration < self.n_iters):
                ...
                if (
                    ...
                    and (
                        self.distance_fn(self.start_state_pos, s) + self.distance_fn(s, self.goal_state_pos)
                        >= goal_n.cost
                    )
                ):
                ...
                new = OptimalNode(
                    path[-1],
                    parent=nearest,
                    d=self.distance_fn(nearest.config, path[-1]),
                    path=list(path[:-1]),
                    iteration=iteration,
                )
    ...
    def print_info(self, iteration, elapsed_time, success, goal_n):
        print(
            f"Iteration: {iteration:5}/{self.n_iters:5} "
