...
from torchkin import Robot
from torchkin.joint import RevoluteJoint, PrismaticJoint, FixedJoint, Link, Joint
...
def wrapper_torchkin_robot_from_urdf_model(urdf_model, dtype: torch.dtype = None, device: DeviceType = None) -> "Robot":
    if dtype is None:
    ...
    def cache_ancestor_non_fixed_joint_ids():
        for link in robot.links:
            if link.parent_joint is not None:
                link._ancestor_links = link.parent_link.ancestor_links + [link.parent_link]
                ancestor_non_fixed_joint_ids = (
                    link.parent_link.ancestor_non_fixed_joint_ids
                    if isinstance(link.parent_joint, FixedJoint)
                    else link.parent_link.ancestor_non_fixed_joint_ids + [link.parent_joint.id]
                )
