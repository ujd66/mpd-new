...
JOINT_NAME_MAP = {"hinge": "revolute", "slide": "prismatic"}
...
class MJCFRobotModel(RobotModel):
    ...
    def build_topology(self, body, node_list, edge_list):  # since there is no parent tag for joint
        ...
        if not body.body:
            return
        for b in body.body:
            edge_list.append({"source": body.name, "target": b.name})
            self.build_topology(b, node_list, edge_list)
    ...
    def get_body_parameters(self, i, link):
        ...
        if i == 0:
            ...
            if not joint:
                ...
                if joint.pos is not None:
                    trans = torch.tensor(joint.pos, dtype=torch.float32, device=self._device)
                else:
                    trans = torch.zeros(3, device=self._device)
        ...
        if link.inertial is not None:
            ...
            body_params["inertia_mat"] = torch.eye(3, 3, device=self._device).unsqueeze(0)
            print(
                "Warning: No dynamics information for link: {}, setting all inertial properties to 1.".format(link.name)
            )
