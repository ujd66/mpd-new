...
class Manifold(object):
    ...
    def __init__(self, dim_M, dim_T, log_map, exp_map, parallel_transport, name, tensor_args=DEFAULT_TENSOR_ARGS):
        self._dim_M = dim_M
        self._dim_T = dim_T
    ...
    def normal_distribution(self, points_in_manifold, **kwargs):
        ...
        sigma = torch.cov(eps_of_mu.T, aweights=weights) + regularization * torch.eye(
            self.dim_T, **self.tensor_args
        )  # Table 2.4
    ...
    def gaussian_product(self, g_list, eps=1e-5, max_iter=100):
        ...
        if k == max_iter - 1:
            Manifold.logger.warning(
                f"Iterative likelihood maximization for mean terminated after reaching {max_iter} iterations"
            )
