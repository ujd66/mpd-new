...
class Frame(object):
    ...
    def inverse(self) -> Frame:
        ...
        return Frame(rot_transpose, -(rot_transpose @ self._trans.unsqueeze(2)).squeeze(2), device=self.device)

    def multiply_transform(self, frame: Frame) -> Frame:
        new_rot, new_trans = multiply_transform(self._rot, self._trans, frame.rotation, frame.translation)
        return Frame(new_rot, new_trans, device=self.device)

    def multiply_inv_transform(self, frame: Frame) -> Frame:
        new_rot, new_trans = multiply_inv_transform(frame.rotation, frame.translation, self._rot, self._trans)
        return Frame(new_rot, new_trans, device=self.device)
    ...
    def get_euler(self) -> torch.Tensor:
        return (
            torch.atan2(self._rot[:, 2, 1], self._rot[:, 2, 2]),
            torch.asin(-self._rot[:, 2, 0]),
            torch.atan2(self._rot[:, 1, 0], self._rot[:, 0, 0]),
        )
