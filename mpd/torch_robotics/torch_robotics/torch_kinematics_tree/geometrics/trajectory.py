...
def smooth_traj(traj, manifold=None, window_length=30, beta=14):
    ...
    if dim_M != manifold.dim_M:
        raise ValueError(
            "[Trajectory]: Input X shape %s is not consistent with manifold.dim_M %s" % (dim_M, manifold.dim_M)
        )
    # apply kaiser filter
...
def compute_traj_velocity(traj, dt, manifold=None):
    ...
    if d_traj.shape[-1] != manifold.dim_T:
        raise ValueError(
            "[Trajectory]: d_traj shape %s is not consistent with manifold.dim_T %s" % (d_traj.shape[0], manifold.dim_T)
        )
    if d_traj.shape[-2] != length:
        raise ValueError(
            "[Trajectory]: Length of d_traj %s is not consistent with input traj length %s" % (d_traj.shape[1], length)
        )
    return d_traj
...
def interpolate(p1, p2, d_param, dist):
    ...
    alpha = min(max(d_param / dist, 0.0), 1.0)
