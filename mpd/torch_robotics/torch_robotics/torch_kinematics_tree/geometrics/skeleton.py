...
class Skeleton:
    ...
    def __init__(self, data, node_list=None, distance_func=None, device="cpu"):
        ...
        self.device = device
        self.node_dist = {
            id: D.MultivariateNormal(
                torch.tensor(self.node_pos[id]).to(device),
    ...
    def compute_link_lengths(self):
        for u, v in self.skeleton.edges():
            u_pos, v_pos = torch.tensor(self.node_pos[u], device=self.device), torch.tensor(
                self.node_pos[v], device=self.device
            )
...
def get_skeleton_from_mediapipe(
    ...
    for idx, landmark in enumerate(landmark_list.landmark):
        if (landmark.HasField("visibility") and landmark.visibility < vis_thres) or (
            landmark.HasField("presence") and landmark.presence < present_thres
        ):
            continue
        if mirror:
    ...
    for connection in connections:
        ...
        if start_idx in plotted_landmarks and end_idx in plotted_landmarks:
            edge_list.append({"source": start_idx, "target": end_idx})
    data = {"nodes": node_list, "links": edge_list}
