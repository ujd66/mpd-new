...
class Gaussian(object):
    ...
    def __init__(self, manifold, mean, cov):
        ...
        if cov.shape[0] != manifold.dim_T or cov.shape[1] != manifold.dim_T:
            raise TypeError(
                f"[Gaussian]: cov shape {cov.shape} and the manifold dim_T {manifold.dim_T, manifold.dim_T} mismatch!"
            )
        self.mean = mean
        self.cov = cov
    ...
    def transform(self, A, b):
        ...
        if A.shape[-2] != self.manifold.dim_T or A.shape[-1] != self.manifold.dim_T:
            raise RuntimeError(
                "[Gaussian]: Expected A to be of the dimension of the tangent"
                " space (%s, %s) and not %s" % (self.manifold.dim_T, self.manifold.dim_T, A.shape)
            )
        if b.shape[-1] != self.manifold.dim_M:
            raise RuntimeError(
                "[Gaussian]: Expected b to be of the dimension of the manifold"
                " space (%s,) and not %s" % (self.manifold.dim_M, b.shape)
            )
        batch_dim = b.shape[:-1]
        mu_trafo = self.manifold.exp_map(A @ (self.manifold.log_map(self.mean)), base=b).view(
            (-1, self.manifold.dim_M)
        )  # (2.53)
        sigma_trafo = self.manifold.matrix_parallel_transport(A @ self.cov @ A.transpose(-2, -1), b, mu_trafo).view(
            (-1, self.manifold.dim_T, self.manifold.dim_T)
        )  # (2.54)
        gaussians = []
