...
def q_exp_map(v, base=None):
    ...
    if base is None:
        ...
        q[non_0] = torch.cat((torch.cos(non_0_norm), (torch.sin(non_0_norm) / non_0_norm) * v[non_0]), dim=-1)
        return q
    else:
...
def q_parallel_transport(p_g, g, h, eps=1e-10):
    ...
    B = torch.cat([torch.zeros((1, 3)), torch.eye(3)], dim=0).to(**tensor_args)
...
def q_to_quaternion_matrix(q):
    q = q.unsqueeze(0) if len(q.shape) == 1 else q
    w, x, y, z = torch.unbind(q, -1)
    o = torch.stack([w, -x, -y, -z, x, w, -z, y, y, z, w, -x, z, -y, x, w], dim=-1)
...
def rotation_matrix_to_q(rot_mat):
    ...
    m00, m01, m02, m10, m11, m12, m20, m21, m22 = torch.unbind(rot_mat.reshape(batch_dim + (9,)), dim=-1)
...
def q_to_axis_angles(q, eps=1e-10):
    ...
    s_half_angles[~beta] = torch.sin(half_angles[~beta]) / angles[~beta]
    s_half_angles[beta] = 0.5 - (angles[beta] * angles[beta]) / 48
...
def axis_angles_to_q(axis_angles, eps=1e-10):
    ...
    s_half_angles[~beta] = torch.sin(half_angles[~beta]) / angles[~beta]
    s_half_angles[beta] = 0.5 - (angles[beta] * angles[beta]) / 48
    q = torch.cat([torch.cos(half_angles), axis_angles * s_half_angles], dim=-1)
    return q
